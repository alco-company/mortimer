module StockLocationsHelper
  ...
  def link_to_location resource, options
    return link_to( resource.asset.name, stock_stock_location_url(resource.stock,resource), options) if parent?
    link_to resource.asset.name, stock_location_url(resource), options
  end
