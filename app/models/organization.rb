class Organization < AbstractResource
  ...
  #   where("participant" deleted_at: nil)
    Organization.all.joins(:participant)
  end
  ...
  def broadcast_update
    if self.participant.deleted_at.nil? 
      broadcast_replace_later_to 'organizations', 
        partial: self, 
        target: self, 
        locals: { resource: self, user: Current.user }
    else 
      broadcast_remove_to 'organizations', target: self
    end
  end

  def broadcast_destroy
    raise "Organization should not be hard-deleteable!"
