...
let heartbeatUrl = "";
let heartbeatTimerTimeout = 600000; //every 10 minuts
  ...
  .then(data => {
    try{
      switch(true){
        ...
        case (data.id.toString() == stockId):         postMessage( {link_up: true} ); break;
...
// // setFrequency
// // interface for setting the timerTimeout - ie how often to pullQueueData
// // 
// function setFrequency(event){
//   try{ 
//     timerTimeout = Number( event.data["frequency"] ); 
//   } catch(err){ 
//     reportError({ message: "frequency", data: event.data, error: err }) 
//   }
// }

// // setHeartBeatFrequency
// // interface for setting the heartbeatTimerTimeout - ie how often to heartbeat 
// //
// function setHeartBeatFrequency(event) {
//   try{ 
//     heartbeatTimerTimeout = Number(event.data["heartbeat_frequency"])
//   } catch(err){ 
//     reportError({ message: "heartbeat_frequency", data: event.data, error: err }) 
//   }
// }

// // setApiKey 
// // interface for setting the apiKey
// //
// function setApiKey(event) {
//   try{ 
//     apiKey = event.data["apiKey"]
//   } catch(err){ 
//     reportError({ message: "apiKey", data: event.data, error: err }) 
//   }
// }
...
onmessage = function(event) {
  switch(true){
    case event.data.hasOwnProperty('url'):                 urlUrl = event.data['url'] // } = JSON.parse(event.data["url"]); console.log(`2 this is the url ${urlUrl}`);
    case event.data.hasOwnProperty('method'):              urlMethod = event.data['method'] // } = JSON.parse(event.data["url"]); console.log(`2 this is the url ${urlUrl}`);
    ...
    case event.data.hasOwnProperty("stockid"):             stockId = event.data['stockid']; console.log(stockId);
